...
SELECT COUNT(*) - COUNT(title)                 AS title_nulls,
...
WITH country_summary
     AS (SELECT SUM(CASE
				...
				SUM(CASE
                      WHEN m.country LIKE '%Italy%' and m.country LIKE '%Germany%' THEN r.total_votes
                    END) AS both_country_total_votes
         FROM   movie m
                LEFT JOIN ratings r
                       ON r.movie_id = m.id)
SELECT CASE
         WHEN (german_total_votes - both_country_total_votes) > (italian_total_votes - both_country_total_votes)  THEN 'Yes'
         ELSE 'No'
       END AS 'Answer'
...
SELECT genre,
       ...
       ROUND(AVG(ROUND(AVG(duration), 2))
               OVER(
                 ORDER BY genre ROWS 12 preceding), 2) AS moving_avg_duration
FROM   movie m
       inner join genre g
               ON m.id = g.movie_id
GROUP  BY genre; 
...
ALTER TABLE movie
  ADD COLUMN worldwide_gross_income_usd VARCHAR(30);

SET sql_safe_updates = 0;

UPDATE movie
SET    worldwide_gross_income_usd = CASE
                                      WHEN worlwide_gross_income LIKE 'INR %'
                                    THEN
                                      CONCAT('$', ROUND(
                                      SUBSTRING(worlwide_gross_income, 5) / 75.0
                                                  , 2))
...
AS
  (
             SELECT     g.genre ,
                        ...
                        DENSE_RANK() over(PARTITION BY YEAR(date_published) ORDER BY worldwide_gross_income_USD DESC) AS movie_rank
             FROM       movie m
             INNER JOIN genre g
             ON         g.movie_id = m.id
             WHERE      g.genre IN
...
AS
  (
         SELECT * ,
                datediff(next_movie_date, date_published) AS movie_date_diff
         FROM   movie_date_summary)
  SELECT     d.name_id                                                        AS director_id,
             n.name                                                           AS director_name,
             count(DISTINCT d.movie_id)                                       AS number_of_movies,
             round(avg(movie_date_diff))                                      AS avg_inter_movie_days,
  ...
  INNER JOIN ratings r
  ON         r.movie_id = d.movie_id
  INNER JOIN inter_movie_days i
  ON         i.name_id = d.name_id
  GROUP BY   d.name_id
